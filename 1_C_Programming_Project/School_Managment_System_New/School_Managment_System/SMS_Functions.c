...
{ /*Beginning Of StudentList(List *lPtr)*/
	...
	{ /*Beginning Of if(lPtr->size==1)*/
		...
		printf("%-21s",tmPtr->entry->stdnt_Nm);
		printf("%-10s",tmPtr->entry->stdnt_gndr);
		printf("%02hu/%02hu/%04hu     ",tmPtr->entry->brthdy.dy,tmPtr->entry->brthdy.mnth,tmPtr->entry->brthdy.yr);
		printf("%-20s",tmPtr->entry->phNumbr);
		printf("%-30s",tmPtr->entry->addrss);
		printf("%-10s",tmPtr->entry->stdnt_grd);
		printf("%-10hu\n",tmPtr->entry->id);
